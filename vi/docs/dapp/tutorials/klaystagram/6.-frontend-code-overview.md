# 6. Tổng quan về mã Frontend <a id="6-frontend-code-overview"></a>
...
`src/klaytn`: Chứa các tệp giúp tương tác với chuỗi khối Klaytn.
...
* `src/pages/FeedPage.js`: Hiển thị ảnh đã đọc từ hợp đồng, hiển thị các ảnh đó tới người dùng và cung cấp tính năng tải lên.

`src/components`: Chứa các tệp thành phần soạn nên trang.
...
`src/App.js`: Tệp thành phần gốc của ứng dụng hướng dẫn dành cho các thành phần tổng thể.

## 2\) App.js <a id="1-app-js"></a>

`'App.js'` là tệp thành phần gốc dành cho các thành phần tổng thể. Nó render hai trang tùy thuộc vào trạng thái đăng nhập của người dùng. Mỗi trang có các hàm tương tác với hợp đồng. Bạn phải thêm phiên bản ví vào caver để gửi giao dịch tới chuỗi khối. Hãy xem xét nhanh mã cho phần tổng quan.

cf. caver-js\(or `cav` in the code\) là một thư viện để tương tác với mã Klaytn blockchain. Chúng ta sẽ tìm hiểu chi tiết trong chương tiếp theo - [7-1. Kết nối hợp đồng tới Frontend](7.-feedpage/7-1.-connect-contract-to-frontend.md)
...
nhập * as authActions từ 'redux/actions/auth'
...
xuất kết nối mặc định(mapStateToProps, mapDispatchToProps)(App)
```

cf\) Ta cần `JSON.parse` vì phần `walletInstance` được lưu trữ là chuỗi JSON.

**1. Khởi chạy trạng thái **`isLoggedIn`  
Để khởi chạy trạng thái `isLoggedIn`, ta dùng phương pháp vòng đời `constructor` trên thành phần ứng dụng. Trạng thái này kiểm tra phần `walletInstance` ở sessionStorage của trình duyệt trước khi thành phần được tạo.

**2. Tích hợp/Xóa ví**  
Nếu bạn chưa từng đăng nhập trước đây, phần `walletInstance` có thể không tồn tại. Nếu bạn từng đăng nhập, phần `walletInstance` có thể tồn tại dưới dạng chuỗi JSON ở sessionStorage.

1. Tích hợp - Nếu phiên bản ví tồn tại ở sessionStorage, hãy thử thêm phiên bản ví vào caver và cửa hàng redux.
...
export const integrateWallet = (privateKey) => (dispatch) => {
  ...
  // Để duy trì trạng thái đăng nhập, hãy lưu trữ walletInstance ở sessionStorage
...
cf. Để biết thêm thông tin về API `privateKeyToAccount` của caver, hãy tham khảo [caver.klay.accounts.privateKeyToAccount](https://docs.klaytn.foundation/dapp/sdk/caver-js/v1.4.1/api-references/caver.klay.accounts#privatekeytoaccount).

**3. Render trang** Redux sẽ khởi chạy trạng thái `isLoggedIn` thành true hoặc false, tùy thuộc vào việc walletInstance có tồn tại trong phần lưu trữ phiên hay không

## 3\) `src/pages` <a id="2-src-pages"></a>

Như ta đã mô tả ở [above](6.-frontend-code-overview.md#1-overview), `src/pages` chứa các tệp hai trang. Một trong những tệp hai trang này sẽ được render trên ứng dụng, tùy thuộc vào việc người dùng đã đăng nhập hay chưa.

* `AuthPage.js`: Chứa mẫu đăng nhập và đăng ký. Bạn có thể tạo khóa riêng tư trong mẫu đăng ký và sử dụng khóa này để đăng nhập trên ứng dụng.
* `src/pages/FeedPage.js`: Đọc dữ liệu ảnh từ hợp đồng và hiển thị tới người dùng. Người dùng cũng có thể tải ảnh của họ lên.
...
Đọc dữ liệu từ hợp đồng không mất phí..  
Mặt khác, bạn sẽ phải trả phí để viết dữ liệu vào hợp đồng \(Gửi giao dịch\). Vì vậy, để viết dữ liệu, bạn phải có tài khoản Klaytn có KLAY để trả phí.
...
Trong hướng dẫn này, ta sẽ tập trung vào `FeedPage`, tìm hiểu về cách ứng dụng này **đọc và viết dữ liệu** từ/vào hợp đồng.
