...
Để khởi chạy trạng thái `isLoggedIn`, chúng ta dùng phương thức vòng đời `constructor` trên thành phần ứng dụng. Trạng thái này kiểm tra phiên `walletInstance` ở sessionStorage của trình duyệt trước khi thành phần được liên kết.
...
cf. Để biết thêm thông tin về API `privateKeyToAccount` của caver, hãy xem [caver.klay.accounts.privateKeyToAccount](https://docs.klaytn.foundation/dapp/sdk/caver-js/v1.4.1/api-references/caver.klay.accounts#privatekeytoaccount).
...
Như chúng ta đã mô tả ở [above](6.-frontend-code-overview.md#1-overview), `src/pages` chứa các tệp hai trang. Một trong những tệp hai trang này sẽ được kết xuất trên ứng dụng, tùy thuộc vào việc người dùng đã đăng nhập hay chưa.
