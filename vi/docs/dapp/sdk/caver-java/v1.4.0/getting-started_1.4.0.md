...
Để cập nhật khóa tài khoản trên nền tảng Klaytn, caver-java cung cấp giao diện `AccountKey`. Nội dung dưới đây mô tả về `AccountKey` implementations, `AccountKeyPublic`, `AccountKeyWeightedMultiSig` và `AccountKeyRoleBased`. Hãy xem [AccountUpdate](#tài khoản-update) để tìm hiểu về cách cập nhật Tài khoản.
