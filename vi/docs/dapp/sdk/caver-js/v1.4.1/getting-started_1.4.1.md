...
Bạn có thể dùng `caver-js` để tạo một tài khoản như dưới đây. Bạn cũng có thể tạo một tài khoản qua [Ví Klaytn](../../../../toolkit/klaytn-wallet.md#create-a-new-tài khoản).
...
Bạn có thể dùng đối tượng promise hoặc lớp phát sinh sự kiện để nhận biên lai của giao dịch trước khi chuyển giao dịch đó đến [caver.klay.sendSignedTransaction](api-references/caver.klay/transaction.md#sendsignedtransaction) hoặc [caver.klay.sendTransaction](api-references/caver.klay/transaction.md#sendtransaction).
...
Klaytn cung cấp nhiều loại giao dịch đa dạng để đẩy mạnh khả năng mở rộng và hiệu suất. Để biết thêm thông tin, hãy xem [Giao dịch](../../../../klaytn/design/transactions/README.md). Mục này mô tả một số ví dụ khác nhau mà bạn có thể sử dụng với caver-js.
...
Tài khoản là một lớp có chứa địa chỉ và khóa của một tài khoản. Một lớp Tài khoản có [AccountKey](#tài khoảnkey), có thể là loại [AccountKeyPublic](#tài khoảnkeypublic), [AccountKeyMultiSig](#tài khoảnkeymultisig), hoặc [AccountKeyRoleBased](#tài khoảnkeyrolebased).

Gói caver.klay.tài khoảns sử dụng [AccountKeyPublic](#tài khoảnkeypublic), theo mặc định sẽ chứa và quản lý chuỗi khóa riêng tư.
...
| transactionKey   | Khóa được sử dụng cho [RoleTransaction](../../../../klaytn/design/tài khoảns.md#roles). AccountKeyPublic hoặc AccountKeyMultiSig không bị ràng buộc với bất kỳ vai trò nào, do đó, transactionKey giữ giá trị giống như các khóa.                                                                                          |
| updateKey        | Khóa được sử dụng cho [RoleAccountUpdate](../../../../klaytn/design/tài khoảns.md#roles). AccountKeyPublic hoặc AccountKeyMultiSig không bị ràng buộc với bất kỳ vai trò nào, vì vậy updateKey giữ giá trị giống như các khóa.                                                                                             |
| feePayerKey      | Khóa được sử dụng cho [RoleFeePayer](../../../../klaytn/design/tài khoảns.md#roles). AccountKeyPublic hoặc AccountKeyMultiSig không bị ràng buộc với bất kỳ vai trò nào, do đó, feePayerKey giữ cùng một giá trị như các khóa.                                                                                             |
...
Nội dung giải thích về các lớp AccountKey khác nhau được cung cấp trong phần [AccountKey](#tài khoảnkey).

### AccountKey  <a id="accountkey"></a>
AccountKey là một cấu trúc dữ liệu có chứa các khóa của một tài khoản. Một tài khoản có thể có một chuỗi khóa riêng tư hoặc nhiều chuỗi khóa riêng tư để sử dụng cho việc ký tên. Tài khoản cũng có thể quản lý các khỏa riêng tư bằng [roles](../../../../klaytn/design/tài khoảns.md#roles).
...
AccountKeyRoleBased lưu trữ và quản lý các khóa theo vai trò, vì thế, nếu bạn chạy ví dụ trên, bạn sẽ thấy ba vai trò (transactionKey, updateKey, feePayerKey) được xác định trong thuộc tính `keys`. Vì thế, không như AccountKey khác ([AccountKeyPublic](#tài khoảnkeypublic) or [AccountKeyMultiSig](#tài khoảnkeymultisig)), từng transactionKey, updateKey và feePayerKey đều tương ứng với một khóa khác nhau.
...
[AccountFỏUpdate](#tài khoảnforupdate) sau đây giải thích cách để cập nhật tài khoản bằng cách gửi một giao dịch đến mạng lưới Klaytn.
...
Các ví dụ dưới đây bắt đầu bằng việc cập nhật tài khoản bằng tài khoảnKey. Trong tài khoản phải có đủ KLAY để dùng khi thử nghiệm. KLAY thử nghiệm cho mạng Baobab có sẵn qua [Vòi Baobab](../../../../toolkit/klaytn-wallet.md#how-to-receive-baobab-testnet-klay).
...
[caver.klay.tài khoảns]: api-references/caver.klay.tài khoảns.md
[caver.klay.tài khoảns.signTransaction]: api-references/caver.klay.tài khoảns.md#signtransaction
...
[txError: Thông tin chi tiết về các lỗi giao dịch]: ../../../json-rpc/transaction-error-codes.md
[Ủy thác phí]: ../../../../klaytn/design/transactions/README.md#fee-delegation
[AccountKey]: ../../../../klaytn/design/tài khoảns.md#tài khoản-key
[Cập nhật tài khoản]: api-references/caver.klay/sendtx_tài khoản_update.md
