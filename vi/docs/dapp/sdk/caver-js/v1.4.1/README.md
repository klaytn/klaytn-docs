...
* [caver.klay.tài khoảns](api-references/caver.klay.accounts.md)
...
Klaytn sử dụng giá gas cố định \(25 ston = 25 \* 10^9\). Một giao dịch có giá khác được gửi lên mạng lưới Klaytn sẽ bị từ chối. Để biết thêm thông tin về giá gas, hãy xem [Tổng quan về đơn giá và giá gas](../../../../klaytn/design/transaction-fees/transaction-fees.md#gas-and-unit-price-overview) Bạn có thể lấy giá gas được dùng trong mạng lưới bằng cách sử dụng [caver.klay.getGasPrice](api-references/caver.klay/config.md#getgasprice).

Nếu `gasPrice` không được xác định khi bạn ký hoặc gửi một giao dịch, caver-js sẽ gọi RPC [caver.klay.getGasPrice](api-references/caver.klay/config.md#getgasprice) để đặt giá gas của giao dịch.
