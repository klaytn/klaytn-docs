...
| 이름 | 유형 | 설명
| ---- | ------ | ------------------------------------------------------------------------------------------------------------------------- |
| size | Number | HEX 문자열의 바이트 크기, _예: `32`는 "0x"가 접두사로 붙은 64개의 문자가 포함된 32바이트 HEX 문자열을 생성합니다. |
...
| 이름 | 유형 | 설명
| ------ | ---------------- | ---------------------------------- |
| number | string \| number | 숫자를 큰 숫자로 변환합니다. |
...
| 유형 | 설명
| ------- | ---------------------------------------------------------------------------------------------------------------------- |
| bool | 주소의 체크섬이 유효하면 `true`, 체크섬 주소가 아니거나 체크섬이 유효하지 않으면 `false`를 반환합니다. |
...
| 이름 | 유형 | 설명
| --------- | ------ | ----------------------------- |
| hexstring | String | 변환할 HEX 문자열입니다. |
...
| 이름 | 유형 | 설명
| ---- | ------ | ------------------------------------------ |
| str | String | HEX 문자열로 변환할 UTF-8 문자열입니다. |
...
| 이름 | 유형 | 설명
| ---- | ------ | ------------------------------------------- |
| str | String | HEX 문자열로 변환할 ASCII 문자열입니다. |
...
| unit | String | <p>(선택 사항, 기본값은 <code>"KLAY"</code>) 변환할 KLAY의 단위입니다.   d제공된 단위에는 다음 승수 중 하나가 곱해집니다. - <code>peb</code>: '1'- <code>kpeb</code>: '1000'- <code>Mpeb</code>: '1000000'- <code>Gpeb</code>: '1000000000'- <code>Ston</code>: '1000000000'- <code>uKLAY</code>: '1000000000000'- <code>mKLAY</code>: '1000000000000000'- <code>KLAY</code>: '1000000000000000000'- <code>kKLAY</code>: '1000000000000000000000'- <code>MKLAY</code>: '1000000000000000000000000'- <code>GKLAY</code>: '1000000000000000000000000000'</p> |.

**리턴 값**

| 유형 | 설명
| ------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| string \| BN | 숫자 매개변수가 [BN](https://github.com/indutny/bn.js/)의 인스턴스인 경우 BN 인스턴스를 반환하고, 그렇지 않은 경우 문자열을 반환합니다. |
...
| 이름 | 유형 | 설명
| ------ | ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| number | string \| number \| BN \| BigNumber | 단위가 peb인 값                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| unit | String | <p>(선택 사항, 기본값은 <code>"KLAY"</code>) "peb"를 변환할 KLAY 단위입니다. 제공된 단위의 분모는 다음 분모 중 하나로 나뉩니다. - <code>peb</code>: '1'- <code>kpeb</code>: '1000'- <code>Mpeb</code>: '1000000'- <code>Gpeb</code>: '1000000000'- <code>Ston</code>: '1000000000'- <code>uKLAY</code>: '1000000000000'- <code>mKLAY</code>: '1000000000000000'- <code>KLAY</code>: '1000000000000000000'- <code>kKLAY</code>: '1000000000000000000000'- <code>MKLAY</code>: '1000000000000000000000000'- <code>GKLAY</code>: '1000000000000000000000000000'</p> |.
...
| 이름 | 유형 | 설명
| --------- | ------ | --------------------- |
| hexString | String | 다듬을 16진수 문자열입니다. |
...
| 이름 | 유형 | 설명
| --------- | ------ | -------------------------- |
| hexString | String | 짝수로 만들 16진수 문자열입니다. |
...
| 유형 | 설명
| ------- | ----------------------------------------------------------------- |
| bool | `true`는 트랜잭션 객체가 스마트 컨트랙트 배포용임을 의미합니다. |
...
주어진 공개키의 x, y 좌표를 반환합니다. 키 암호화에 대한 자세한 내용은 [타원 곡선 암호화](https://en.wikipedia.org/wiki/Elliptic-curve_cryptography)를 참조하세요.
...
| 유형 | 설명
| ------- | ------------------------------------------------- |
| bool | `true`는 입력이 0x 접두사가 붙은 16진수 문자열임을 의미합니다. |
...
| 유형 | 설명
| ------- | --------------------------------------------------------- |
| bool | `true`는 입력이 32바이트 해시 형식임을 의미합니다. |
...
| 유형 | 설명
| ------- | --------------------------------------------------------------------- |
| bool | `true`는 입력이 0x 접두사가 붙은 32바이트 해시 형식임을 의미합니다. |
...
입력이 트랜잭션 해시 형식이면 `true`를 반환하고, 그렇지 않으면 `false`를 반환합니다. 이 함수는 입력만 살펴보고 트랜잭션 해시 형식인지 여부를 판단합니다.
...
| 유형 | 설명
| ------- | ------------------------------------------------------------ |
| bool | `true`는 입력이 트랜잭션 해시 형식임을 의미합니다. |
...
입력이 트랜잭션 해시 형식이면 `true`를 반환하고, 그렇지 않으면 `false`를 반환합니다. 이 함수는 입력만 보고 트랜잭션 해시 형식인지 여부를 판단합니다. [caver.utils.isTxHash](#istxhash)와 다른 점은 HEX 앞에 `0x`가 붙을 것으로 예상한다는 점입니다.
...
| 유형 | 설명
| ------- | ----------------------------------------------------------- |
| bool | `true`는 사전 정의 블록이 유효한 사전 정의 블록 태그임을 의미합니다.
...
| 유형 | 설명
| ------- | ----------------------------------------------------------------------------------------------------------------- |
| bool | `true`는 키가 `0x{private key}0x{type}0x{address in hex}` 또는 `{private key}0x{type}0x{address in hex}`라는 의미입니다. |
...
## bufferToHex <a href="#isklaytnwalletkey" id="isklaytnwalletkey"></a>
...
## parseKlaytnWalletKey <a href="#buffertohex" id="buffertohex"></a>
