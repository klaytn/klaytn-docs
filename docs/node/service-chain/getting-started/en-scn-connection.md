...
## Step 4: Configure SCN and restart kscn <a id="step-4-configure-scn-then-restart-kscn"></a>
From the SCN-L2-01 node's shell, edit `kscn-XXXXX-amd64/conf/kscnd.conf`.
If `SC_ANCHORING` is set to 1, data anchoring starts automatically when the SCN-L2-01 node starts. In this example, `SC_PARENT_CHAIN_ID` is set to 1001 because the `chainID` of the parent chain, Baobab, is 1001.
...
You can see the automatically created child and operator keystores (`parent_bridge_account` and `child_bridge_account`) in the `data` directory.
Copy the `parent_bridge_account` keystore directory to the `data` directory on the EN machine.
The keystore must be the same keystore with the child bridge node uses.
Note that the `parent_bridge_account` directory must have keystore files. If not, the EN process would be immediately terminated.

Of course, you can use another keystore file, which is not generated by SCN node.
If so, you have to place the keystore files to each keystore(`child_bridge_account` or `parent_bridge_account`) directory you want to use your pre-generated keys.

For now, we are terminating the SCN process and we have further steps for the residual configuration.
```
SCN-L2-01$ kscnd stop
Shutting down kscnd: OK
```

## Step 5: Configure mainbridge feature on EN configuration. <a id="step-5-set-mainbridge-option"></a>
Open the `kend.conf` file and insert `SC_MAIN_BRIDGE=1` option.
```
...
SC_MAIN_BRIDGE=1
...
...
## Step 6: Start the EN Node <a id="step-6-start-the-en-node"></a>
```
EN-01$ kend start
Starting kscnd: OK
```
...
## Step 7: Check KNI of EN Node <a id="step-7-check-kni-of-en-node"></a>
...
## Step 8: Create main-bridges.json <a id="step-8-create-main-bridges-json"></a>
...
## Step 9: Restart KSCN node <a id="step-9-restart-kscn"></a>
From the SCN-L2-01 node's shell, restart kscn by executing the following command:
```
SCN-L2-01$ kscnd start
Starting kscnd: OK
```
