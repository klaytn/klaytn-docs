...
Klaytn v1.7.2 fixed a race condition issue that may cause a panic when CNs process txs and generate a block simultaneously. This patch is only for a CN-specific issue. Other node types (EN, PN) do not need to update the Klaytn client to this version.
