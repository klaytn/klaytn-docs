# Fee Delegation Example <a id="fee-delegation-example"></a>
...
To sign a transaction, use [signTransaction](../sdk/caver-js/api-references/caver.klay.accounts.md#signtransaction) which signs a transaction with given private key.
...
### 2.2 Transaction signing by the fee payer <a id="2-2-transaction-signing-by-the-fee-payer"></a>
...
## 3. Simple server and client for fee delegation <a id="3-simple-server-and-client-for-fee-delegation"></a>

Let's write a simple server and client using above fee delegation code.

### 3.1 Environment setup <a id="3-1-environment-setup"></a>

We will use `npm` and [caver-js](../sdk/caver-js/README.md) to setup environment for this example as below.
...
### 3.1 Sender's client <a id="3-1-sender-s-client"></a>
...
## 4. Run example <a id="4-run-example"></a>

Prepare two terminals, one for `sender_client.js` and another for `feepayer_server.js`.

### 4.1 Run `feepayer_server.js` <a id="4-1-run-feepayer_server-js"></a>
...
### 4.2 Run `sender_client.js` <a id="4-2-run-sender_client-js"></a>
...
### 4.4 Klaytn scope <a id="4-4-klaytn-scope"></a>
