...
Encode an account key using the Recursive Length Prefix (RLP) encoding scheme. 

**Parameters**
| Name | Type | Description |
| --- | --- | --- |
| keytype | QUANTITY | Integer value indicating account key type. For the expect value of each account key type, see [Account Key](../../../../klaytn/design/accounts#account-key). |
| key | JSON DATA | Account key to be encoded. |
...
```
```shell
// Request AccountKeyWeightedMultiSig
...
Decode an RLP encoded account key.  
