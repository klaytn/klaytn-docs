...
| options.fromBlock | Number &#124; String | (optional) The number of the earliest block (``"latest"`` may be given to mean the most recent and ``"pending"`` currently mining, block). By default ``"latest"``. |
| options.toBlock | Number &#124; String | (optional) The number of the latest block (``"latest"`` may be given to mean the most recent and ``"pending"`` currently mining, block). By default ``"latest"``. |
