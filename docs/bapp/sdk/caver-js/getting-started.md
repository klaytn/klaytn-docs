...
You can use the promise or event emitter to get the receipt of the transaction when you transfer the transaction to Klaytn network by [caver.rpc.klay.sendRawTransaction].
...
When the above code is executed, the `RLP-encoded string` including the sender's `signatures` and fee payer's `feePayerSignatures` is printed like below. (The output you got could be different from the string output shown below.)
...
To run this example, you need to update the accountKey of account you use for testing with [AccountKeyWeightedMultiSig]. Please refer to [Account Update](#account-update) for how to update your account.
...
async function testFunction() {
	...
	const combined = vt.combineSignatures(rlpEncodedStrings)
