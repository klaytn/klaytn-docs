...
You can use `caver-js` to create an account as shown below. You can also create an account via [Klaytn Wallet](getting-started.md).
...
**Note**: Functions associated with [caver.klay.accounts](getting-started.md) have no effect on the actual Klaytn network.
...
You can get a RLP-encoded transaction \(`rawTransaction`\) using [caver.klay.accounts.signTransaction](getting-started.md) as above and use this to transfer the transaction to the Klaytn network as below.
...
As described in the example above, you can get the result of sending a transaction through the promise and event emitter. And also, if you know the transaction hash, you can query the transaction receipt using the [caver.klay.getTransactionReceipt](getting-started.md) RPC call. The example below shows how to get a receipt using the [caver.klay.getTransactionReceipt](getting-started.md) RPC call.
...
The result of the transaction can be found through the status of the receipt. For a detailed description of the return values, see [getTransactionReceipt](getting-started.md). If a transaction is failed, you can check the detailed error in `txError` of the receipt. For more information about `txError`, see [txError: Detailed Information of Transaction Failures](getting-started.md).
...
Klaytn provides [Fee Delegation](getting-started.md) feature. Here's an example code.
...
If you want to change the key of the account, send a transaction as shown below. Please check [Account Update](getting-started.md) for the transaction field according to the key type.
...
For smart contract deployment, you can use [caver.klay.Contract](getting-started.md) to deploy it, or you can deploy it using [SMART\_CONTRACT\_DEPLOY](getting-started.md) transaction. Here is an example of using [caver.klay.Contract](getting-started.md).
...
One way to invoke a specific method of a smart contract is to use it with `caver.klay.Contract` or use [SMART\_CONTRACT\_EXECUTION](getting-started.md).
...
See [caver.klay.Contract](getting-started.md) for details.
