...
- If the contract is successfully deployed, you will see the result in the terminal. 

- You can interact with the contract by clicking on the function buttons. Red buttons (`kill` in the example) represent function that change the state on the blockchain and consumes gas. Blue buttons (`greet` in the example) represent functions that do not change the state and do not cost gas. 
