...
| Type   | Description                                                                                                               |
| ------ | ------------------------------------------------------------------------------------------------------------------------- |
| Object | A transaction object. 반환되는 트랜잭션 객체가 가지는 속성은 트랜잭션 타입에 따라 다릅니다. 각 트랜잭션 타입의 속성은 [ SendTransaction](#sendtransaction)을 참조하세요. |
...
| senderTxHash       | 32-byte DATA | (optional) Hash of the tx without the fee payer's address and signature. 이 값은 수수료를 위임하지 않은 트랜잭션의 `hash` 값과 항상 동일합니다.                                                                                         |
...
| Name         | Type     | Description                                                                                                                                                        |
| ------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| senderTxHash | String   | Hash of a transaction that is signed only by the sender. 이에 대한 자세한 내용은 [SenderTxHash](../../../../../../klaytn/design/transactions/README.md#sendertxhash)를 참고하세요. |
| callback     | Function | (optional) Optional callback, returns an error object as the first parameter and the result as the second.                                                         |
...
| Name               | Type     | Description                                                                                                |
| ------------------ | -------- | ---------------------------------------------------------------------------------------------------------- |
| hashStringOrNumber | String   | 블록 번호 또는 해시. Or the string `"genesis"` or `"latest"`.                                                      |
...
| nonce              | QUANTITY       | The number of transactions made by the sender prior to this one.                                                                                                                                             |
| senderTxHash       | 32-byte DATA   | (선택사항) 발신자만 서명한 트랜잭션 해시. See [SenderTxHash](../../../../../../klaytn/design/transactions/README.md#sendertxhash). 이 값은 수수료를 위임하지 않은 트랜잭션의 `transactionHash` 값과 항상 동일합니다.                                     |
| signatures         | Array          | An array of signature objects. A signature object contains three fields (V, R, and S). V contains ECDSA recovery id. R contains ECDSA signature r while S contains ECDSA signature s.                        |
| status             | Boolean        | 트랜잭션이 성공적으로 실행되면 `true`를 반환하며, 만약 Klaytn 가상머신이 트랜잭션을 거부하면 `false`를 반환합니다.                                                                                                                                    |
...
**NOTE**: The receipt is not available for pending transactions. Please note that this API returns correct result only if the indexing feature is enabled in the node by `--sendertxhashindexing`. [isSenderTxHashIndexingEnabled](./config.md#issendertxhashindexingenabled)를 호출하여 인덱싱 기능이 활성화되어 있는지 확인할 수 있습니다.
