...
`approve`는 ERC-20의 필수 메소드입니다. `approve` allows `spender` to withdraw from your account multiple times, up to the `amount`. If this function is called multiple times, it simply resets the allowance to `amount`.
