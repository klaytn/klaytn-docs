...
| 명칭     | 형식 | 설명                                                                                                                                                                                                         |        |    |                                                                    |        |                                                                                                                                                                                                                              |
|:------ |:-- |:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |:------ |:-- |:------------------------------------------------------------------ |:------ |:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| paramX | 복합 | 모든 타입, 또는 `{type: 'uint', value: '123456'}` or `{t: 'bytes', v: '0xfff456'}` 객체. Basic types are autodetected as follows:  - `string` non numerical UTF-8 string is interpreted as `string`.  - \`string | number | BN | HEX`positive number is interpreted as`uint256`.<br> -`string | number | BN`negative number is interpreted as`int256`.<br> -`boolean`as`bool`.<br> -`string`HEX string with leading`0x`is interpreted as`bytes`.<br> -`HEX`HEX number representation is interpreted as`uint256\`. |
...
Returns `true` if the input is in transaction hash format, otherwise it returns `false`. 이 함수는 입력값만 보고 트랜잭션 해시 형식인지를 판별합니다. Difference to [caver.utils.isTxHash](caver.utils.md#istxhash) is that it expects HEX to be prefixed with `0x`.
...
| 명칭  | 형식  | 설명                                                                                          |
|:--- |:--- |:------------------------------------------------------------------------------------------- |
| key | 문자열 | A [KlaytnWalletKey](../../../../klaytn/design/accounts.md#klaytn-wallet-key-format) string. |
...
| 서명 값입니다. | object  | An instance of [SignatureData](caver.wallet/keyring.md#signaturedata).                                                                                                                                                           |
