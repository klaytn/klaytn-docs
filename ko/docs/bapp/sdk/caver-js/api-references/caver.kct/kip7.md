...
| feePayer      | string                                          | (optional) 트랜잭션 수수료를 부담하는 fee payer의 주소입니다. When `feeDelegation` is `true`, the value is set to the `feePayer` field in the transaction. 미입력시 `kip7.options.feePayer`를 사용합니다. 둘 다 미입력시 오류를 반환합니다.                 |
