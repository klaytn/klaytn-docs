...
Klaytn Wallet is a browser-based account management tool for blockchain application \(BApp\) developers on Klaytn. Klaytn Wallet은 Klaytn 노드를 로컬로 실행하지 않고도 개발자가 웹 브라우저를 통해 새 계정을 만들거나 기존 계정의 정보를 직접 볼 수 있도록 합니다. Klaytn Wallet을 통해 사용자가 테스트 목적으로 KLAY 또는 Klaytn 토큰을 다른 계정으로 전송할 수 있습니다.
...
두 번째 단계에서는 설정한 비밀번호로 암호화된 키스토어 파일을 다운로드합니다. `Download & Next Step` 버튼을 클릭하여 키스토어 파일을 바로 다운로드하고 마지막 단계로 이동하세요. \(Note that if the downloaded keystore file gets lost, you can download a new keystore file in the `View Account Info` menu.\)
