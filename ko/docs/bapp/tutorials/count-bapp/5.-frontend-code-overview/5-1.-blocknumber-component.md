...
`BlockNumber` 컴포넌트는 Klaytn의 현재 블록 번호를 보여주는 역할을 합니다. 이 컴포넌트는 `caver.klay.getBlockNumber()`를 매초 호출하여 Klaytn 노드에게 현재 블록 번호를 요청합니다. 응답을 받으면 `this.setState({ currentBlockNumber: blockNumber })`를 통해 DOM을 다시 렌더링합니다.
