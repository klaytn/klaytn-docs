...
After transfering ownership, FeedPhoto needs to be rerendered with new owner's address. To update new owner's address, let's call `feed` data from store and find the photo that has the tokenId from the receipt. 그다음 새 소유자의 주소를 사진의 `OWNER_HISTORY`와 setFeed에 넣습니다.
