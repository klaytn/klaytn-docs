...
`src/styles`: 스타일시트와 관련한 전반적인 스타일 정의를 담고 있습니다.  
`static/`: 이미지나 폰트와 같으느 정적 파일들을 담고 있습니다.  
`src/index.js`: 어플리케이션의 인덱스 파일입니다. ReactDOM.render 로직이 여기에 있습니다.  
`src/App.js`: 어플리케이션의 루트 컴포넌트 파일입니다.