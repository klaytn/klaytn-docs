...
`myVotingPower` 속성은 노드가 보유한 의결권을 나타냅니다. 한 노드당 1.0 ~ 2.0 사이의 의결권을 가집니다. `"none"`, `"single"` 거버넌스 모드에서는 `totalVotingPower` 속성을 통해 제공하는 정보가 없습니다.
...
`itemsAt`은 특정 블록에서의 거버넌스 항목을 반환합니다. 해당 블록의 이전 투표 결과이며, 입력으로 받은 블록 번호에서 체인의 환경설정을 하는 데에 사용됩니다.
...
`pendingChanges`은 충분한 표를 받았지만, 아직 확정되지 않은 항목들의 목록을 반환합니다. 이 항목들은 현재 투표 주기가 끝날 때 확정되며, 그 결과가는 다다음 투표 주기부터 적용됩니다.
