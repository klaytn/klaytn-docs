...
패스프레이즈는 그대로 놔두고 잠금 해제 기간을 따로 설정하려면 패스프레이즈 매개변수를 `null`로 입력하세요.
...
| 명칭         | 형식  | 설명                                                                                                     |
| ---------- | --- | ------------------------------------------------------------------------------------------------------ |
| tx         | 문자열 | 트랜잭션 객체입니다. `from`은 반드시 입력해야 하는 필드입니다. `to`, `value`, `gas`, `gasPrice` and `nonce`는 선택적으로 입력하는 필드입니다. |
| passphrase | 문자열 | `tx.from`의 개인키를 복호화하기 위한 패스프레이즈입니다.                                                                    |
