...
| evmdisTracer   | evmdisTracer는 evmdis 형식의 디스어셈블러에서 얻은 정보를 반환합니다.                                                             |
...
효율성을 위해 동일한 `log` 객체를 각 실행 단계에서 재사용하며 현재 값으로 업데이트합니다. 따라서 현재 호출 이후에도 보존하려는 값은 복사해놓아야 합니다. 예를 들어 아래와 같은 함수는 작동하지 않습니다.
