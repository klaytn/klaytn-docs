...
| subscribed   | 불리언          | `true` if the pair of bridge contracts is subscribed, `false` otherwise. |
...
| isSubscribed     | 불리언           | `true` if the bridge contract is subscribed, `false` otherwise.                                                         |
| onServiceChain   | 불리언           | `true` if the bridge contact is on child (service) chain, `false` otherwise.                                            |
