...
Klaytnscope is the block explorer for the Klaytn Network. Klaytnscope는 네트워크 상태를 모니터링하고 다양한 통계를 제공하여 Klaytn 네트워크에 대해 이해할 수 있게 도와줍니다. 또한 블록 및 트랜잭션 데이터와 Klaytn 네트워크의 스마트 컨트랙트 리스트도 탐색할 수 있습니다.
...
최근 실행된 트랜잭션의 목록입니다. To update the information, please click the refresh.
...
* Time: 트랜잭션 실행으로부터 지금까지 지난 시간입니다. You can check the exact date and time by hovering this.
* From -&gt; To: 트랜잭션 발신자와 수신자의 주소입니다. By clicking the address, you can easily go to the details page. 주소 옆에 파일 아이콘이 표시되면 해당 주소는 컨트랙트의 주소입니다.
...
* Block Size: 블록의 바이트 단위 크기입니다. The more transactions are included, the larger the block size.
...
* Validators: 블록을 검증하는 컨센서스 노드입니다. By clicking the address, you can easily go to the detail view of the node.
...
* Block \#: Number of the block which contains this transaction. 번호를 클릭하여 해당 블록의 상세 보기 페이지로 이동할 수 있습니다.
* From -&gt; To: The addresses of sender and receiver. 이 주소를 클릭하여 계정의 상세 보기 페이지로 쉽게 이동할 수 있습니다. 주소 옆에 파일 아이콘이 표시되면 해당 주소는 컨트랙트의 주소입니다.
...
* TX Fee: The actual cost used to process transaction. Gas Price와 Gas Used 값의 곱으로 계산합니다.
...
* Contract Creator: 컨트랙트를 배포한 계정입니다. By clicking the address, you can go to the detail view of the account.
