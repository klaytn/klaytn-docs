...
This version contains a fix that enhances Metamask compatibility. It affects the return format of block header API. So, it is recommended to update ENs using `eth_getHeader` related APIs to this version.

## Fixes
 - Optional baseFeePerGas field in a block header API returns depends on IsEthTxTypeForkEnabled (#1247)
