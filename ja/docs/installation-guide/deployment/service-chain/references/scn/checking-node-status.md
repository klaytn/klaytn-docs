...
Klaytn は CLI クライアントを提供します: `kscn コンソール`。 Another way of using the client is to connect to the process via IPC (inter-process communication). IPCファイル `klay.ipc` は、SCN の `data` ディレクトリにあります。
